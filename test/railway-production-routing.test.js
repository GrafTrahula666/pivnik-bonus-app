...
test('release gate resolves live service domains and does not contain dead production hosts', async () => {
  const [workflow, deployer, envExample] = await Promise.all([
  ...
  for (const source of [workflow, envExample]) {
    assert.doesNotMatch(source, /pivnik-bonus-app-production\.up\.railway\.app/);
    assert.doesNotMatch(source, /pivnik-vk-test-production\.up\.railway\.app/);
  }
  assert.match(workflow, /railway-production-auth-smoke\.mjs/);
