...
test('optimized admin summary SQL preserves KPI semantics on real PostgreSQL aggregates', async () => {
  const db = new PGlite();
  try {
    await db.exec(`
      CREATE TABLE users (
      ...
      INSERT INTO transactions (
        client_id, status, bonus_earned, bonus_spent, check_amount_cents,
        is_suspicious, created_at, cancelled_at
      ) VALUES
    ...
    const sql = await loadAdminSummarySql();
    ...
    assert.deepEqual({
      ...
      newClients7d: Number(row.new_clients_7d),
      activeClients30d: Number(row.active_clients_30d),
      inactiveClients30d: Number(row.inactive_clients_30d),
      issued: Number(row.issued),
      redeemed: Number(row.redeemed),
      todayOps: Number(row.today_ops),
      todayCheckCents: Number(row.today_check_cents),
      yesterdayCheckCents: Number(row.yesterday_check_cents),
      lifetimeCheckCents: Number(row.lifetime_check_cents),
      suspiciousOps: Number(row.suspicious_ops),
      cancelledToday: Number(row.cancelled_today)
    }, {
      clients: 4,
      newClients7d: 1,
      activeClients30d: 1,
      inactiveClients30d: 1,
      issued: 110,
      redeemed: 11,
      todayOps: 3,
      todayCheckCents: 4000,
      yesterdayCheckCents: 2000,
      lifetimeCheckCents: 11000,
      suspiciousOps: 1,
      cancelledToday: 1
    });
  } finally {
