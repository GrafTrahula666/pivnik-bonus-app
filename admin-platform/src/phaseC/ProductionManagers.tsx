    ...
    <section className="card editor-card"><CardTitle title="Призы" action="+ Добавить приз" onAction={()=>setDraft({...draft,prizes:[...draft.prizes,{code:newCode('prize'),title:'Новый приз',rewardType:'bonus',rewardValue:{amount:100},probability:'0',inventoryLimit:null,enabled:true,sortOrder:draft.prizes.length}]})}/>
      <div className="prize-list"><div className="prize-head"><span>Приз</span><span>Тип</span><span>Награда</span><span>Вероятность</span><span>Лимит</span><span>Статус</span><span/></div>
        {draft.prizes.map((p,i)=><div className="prize-row" key={p.code}>
          ...
          <select value={p.rewardType} onChange={e=>update(i,{rewardType:e.target.value})}>{['bonus','beer_ml','item','frame','retry','none'].map(x=><option key={x}>{x}</option>)}</select>
...
export function AchievementManager({venue,session}:{venue:ApiVenue;session:AdminSession}){
  ...
  return <div className="page">
    ...
    <div className="achievement-grid">{items.map((x,i)=><article className="achievement-card card" key={x.code}>
      <div className={cn('achievement-icon',`rarity-${i%4}`)}><Trophy/></div><div className="grow"><div className="achievement-title"><span className="eyebrow">{x.conditionType}</span><Status value={x.enabled?'Активен':'Пауза'}/></div><h3>{x.title}</h3><p>{x.description}</p><div className="achievement-meta"><span>Условие · {x.thresholdValue??'—'}</span><span>Награда · {JSON.stringify(x.rewardValue||{})}</span><span>{x.visibility==='hidden'?'Скрытое':'Публичное'}</span></div></div>
    ...
    {edit&&<EditorModal title="Редактор достижения" onCancel={()=>setEdit(null)} onSave={()=>{const next=items.some(x=>x.code===edit.code)?items.map(x=>x.code===edit.code?edit:x):[...items,edit];setItems(next);setEdit(null)}}>
      ...
      <label className="field"><span>Тип условия</span><select value={edit.conditionType} onChange={e=>setEdit({...edit,conditionType:e.target.value})}>{['purchase_count','single_check','lifetime_spend','wheel_spin','shop_purchase','manual'].map(x=><option key={x}>{x}</option>)}</select></label>
...
export function ShopManager({venue,session}:{venue:ApiVenue;session:AdminSession}){
  ...
  return <div className="page"><PageHead eyebrow="КАТАЛОГ НАГРАД" title="Магазин" sub={`${venue.companyName} → ${venue.name}`}
    ...
    <div className="shop-grid">{items.map((x,i)=><article className="product-card card" key={x.code}><div className={cn('product-art',`art-${i%4}`)}><ShoppingBag/><span>{x.category}</span><button className="icon-btn" onClick={()=>setEdit({...x})}><Pencil/></button></div>
    ...
    {edit&&<EditorModal title="Редактор товара" onCancel={()=>setEdit(null)} onSave={()=>{const next=items.some(x=>x.code===edit.code)?items.map(x=>x.code===edit.code?edit:x):[...items,edit];setItems(next);setEdit(null)}}>
      ...
      <label className="field"><span>Тип награды</span><select value={edit.rewardType} onChange={e=>setEdit({...edit,rewardType:e.target.value})}>{['item','digital_reward','frame','bonus'].map(x=><option key={x}>{x}</option>)}</select></label>
...
export function PromotionManager({venue,session}:{venue:ApiVenue;session:AdminSession}){
  ...
  return <div className="page"><PageHead eyebrow="МАРКЕТИНГОВЫЕ КАМПАНИИ" title="Акции" sub={`${venue.companyName} → ${venue.name}`}
    ...
    <div className="promo-grid">{items.map((x,i)=><article className="promo-card card" key={x.code}><div className={cn('promo-visual',`promo-${i%3}`)}><TicketPercent/><span>{x.state||'DRAFT'}</span></div><div className="promo-body">
      <div><Status value={x.state||'DRAFT'}/><button className="ghost-icon" onClick={()=>setEdit({...x})}><Pencil/></button></div><h3>{x.title}</h3><p>{x.description}</p><strong>{x.multiplier?`×${x.multiplier}`:JSON.stringify(x.reward||{})}</strong><div className="promo-metrics"><span>{x.startsAt?new Date(x.startsAt).toLocaleString('ru-RU'):'без старта'}</span><span>{x.endsAt?new Date(x.endsAt).toLocaleString('ru-RU'):'без конца'}</span></div>
...
export function BrandingManager({venue,session}:{venue:ApiVenue;session:AdminSession}){
  ...
  return <div className="page"><PageHead eyebrow="ФИРМЕННЫЙ СТИЛЬ" title="Оформление" sub={`${venue.companyName} → ${venue.name}`}
    ...
    <div className="brand-layout"><section className="card editor-card brand-form"><CardTitle title="Идентика"/>
      ...
      <Field label="Тема оформления" value={String(draft.branding.theme||'dark')} onChange={v=>setDraft({...draft,branding:{...draft.branding,theme:v}})}/><Field label="Телефон" value={draft.phone||''} onChange={v=>setDraft({...draft,phone:v})}/>
