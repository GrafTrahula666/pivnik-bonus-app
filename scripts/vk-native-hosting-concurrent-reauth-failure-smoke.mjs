...
const outDir = path.join(root, 'artifacts', 'vk-native-hosting-concurrent-reauth-failure-smoke');
const port = 4200;
const gatewayHost = 'vk-gateway.invalid';
const vkUserId = '4242';
const token = 'reauth-failure-session';
const storageKey = `pivnik_vk_${vkUserId}_session`;
const signedLaunchQuery = `vk_app_id=54694987&vk_user_id=${vkUserId}&vk_ts=666666&vk_platform=mobile_iphone&sign=reauth-failure-sign`;

const profile = {
  id: vkUserId, firstName: 'VK Reauth', lastName: 'Failure', username: 'vk_reauth_failure', provider: 'vk', role: 'user',
...
const payload = { token, profile, statuses: [], design: null, promotions: [], shopItems: [], achievements: [], transactions: [], walletConfig: null, leaderboard: { entries: [], currentUser: null } };
...
await page.route('**/*', async (route) => {
  ...
  if (requestUrl.hostname === '127.0.0.1') {
    if (requestUrl.pathname.endsWith('/vendor/vk-bridge.js')) {
      await route.fulfill({ status: 200, contentType: 'text/javascript; charset=utf-8', body: `window.vkBridge={send:async function(method){await window.__recordVkBridgeCall(method);if(method==='VKWebAppGetLaunchParams')return {vk_app_id:54694987,vk_user_id:${vkUserId},vk_ts:666666,vk_platform:'mobile_iphone',sign:'reauth-failure-sign'};if(method==='VKWebAppGetUserInfo')return {id:${vkUserId},first_name:'VK',last_name:'Reauth',photo_200:''};return {};}};` });
      return;
    }
  ...
  if (method === 'POST' && pathname === '/api/auth') {
    ...
    if (expired) return void await route.fulfill(json({ error: 'auth temporarily unavailable' }, 503));
    return void await route.fulfill(json(payload));
  }
  if (expired && authorization === `Bearer ${token}` && (pathname === '/api/me/transactions' || pathname === '/api/achievements' || pathname === '/api/test-mutation')) {
...
try {
  ...
  assert(await page.evaluate((key) => localStorage.getItem(key), storageKey) === token, 'initial scoped session missing');
  ...
  assert(results.every((result) => result.rejected), `all requests must reject when reauth fails: ${JSON.stringify(results)}`);
  ...
  assert(ui.session === null, `expired session must be cleared after failed reauth: ${ui.session}`);
  assert(!ui.appShellHidden, 'failed background recovery unexpectedly closed app shell');
  assert(ui.clientName.includes('VK Reauth'), `existing UI state was lost: ${ui.clientName}`);
  ...
  await page.screenshot({ path: path.join(outDir, 'concurrent-reauth-failure.png'), fullPage: true });
  console.log(JSON.stringify({ ok: true, authCalls: authCalls.length, transactionCalls: transactionCalls.length, achievementCalls: achievementCalls.length, mutationCalls: mutationCalls.length }, null, 2));
} catch (error) {
  ...
  try { await page.screenshot({ path: path.join(outDir, 'concurrent-reauth-failure-failed.png'), fullPage: true }); } catch (_) {}
  throw error;
} finally {
  await context.close();
  await browser.close();
