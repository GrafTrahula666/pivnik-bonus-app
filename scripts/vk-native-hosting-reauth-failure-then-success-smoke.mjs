...
const outDir = path.join(root, 'artifacts', 'vk-native-hosting-reauth-failure-then-success-smoke');
const port = 4202;
const gatewayHost = 'vk-gateway.invalid';
const vkUserId = '4242';
const oldToken = 'old-failed-session';
const newToken = 'new-recovered-session';
const storageKey = `pivnik_vk_${vkUserId}_session`;
const signedLaunchQuery = `vk_app_id=54694987&vk_user_id=${vkUserId}&vk_ts=888888&vk_platform=mobile_iphone&sign=reauth-success-sign`;
const profile = { id: vkUserId, firstName: 'VK Recovery', lastName: 'Success', username: 'vk_recovery_success', provider: 'vk', role: 'user', balance: 777, termsAccepted: true, onboardingComplete: true, photoUrl: '', avatarSource: 'preset_male', avatarKey: null, profileFrame: 'none', monthlySpendCents: 0, totalSpendCents: 0, totalLiters: 0, beerProgressLiters: 0, beerGiftLiters: 0, status: { code: 'traveler', name: 'Путник', bonusPercent: 5, monthlySpendCents: 0, nextSpendCents: 1000000 }, privacy: { publicProfile: true, showName: true, showAvatar: true, showMonthlySpend: true, showStats: true } };
...
await page.route('**/*', async (route) => {
  ...
  if (requestUrl.hostname === '127.0.0.1') {
    if (requestUrl.pathname.endsWith('/vendor/vk-bridge.js')) {
      await route.fulfill({ status: 200, contentType: 'text/javascript; charset=utf-8', body: `window.vkBridge={send:async function(method){await window.__recordVkBridgeCall(method);if(method==='VKWebAppGetLaunchParams')return {vk_app_id:54694987,vk_user_id:${vkUserId},vk_ts:888888,vk_platform:'mobile_iphone',sign:'reauth-success-sign'};if(method==='VKWebAppGetUserInfo')return {id:${vkUserId},first_name:'VK',last_name:'Recovery',photo_200:''};return {};}};` });
      return;
    }
  ...
  if (method === 'POST' && pathname === '/api/auth') {
    authCount += 1;
    if (expired && !allowSuccessfulAuth) return void await route.fulfill(json({ error: 'auth temporarily unavailable' }, 503));
    return void await route.fulfill(json(payload(allowSuccessfulAuth ? newToken : oldToken)));
  }
  ...
  if (pathname === '/api/bootstrap' || pathname === '/api/me') return void await route.fulfill(json(payload(allowSuccessfulAuth ? newToken : oldToken)));
...
try {
  ...
  expired = true;
  const failed = await page.evaluate(async () => window.api('/api/me/transactions').then(() => ({ rejected: false }), (error) => ({ rejected: true, status: Number(error?.status || 0), message: String(error?.message || '') })));
  assert(failed.rejected && failed.status === 503, `failed recovery must surface 503: ${JSON.stringify(failed)}`);
  assert(authCount === 3, `failed recovery must add exactly one auth call: ${authCount}`);
  assert(await page.evaluate((key) => localStorage.getItem(key), storageKey) === null, 'failed recovery must leave old session cleared');

  allowSuccessfulAuth = true;
  await page.evaluate(async () => { await window.authenticate(); });
  assert(authCount === 4, `successful manual auth must add exactly one auth call: ${authCount}`);
  assert(await page.evaluate((key) => localStorage.getItem(key), storageKey) === newToken, 'successful auth did not persist new scoped session');

  const followUp = await page.evaluate(async () => window.api('/api/me/transactions'));
  assert(Array.isArray(followUp.transactions), `follow-up GET did not succeed: ${JSON.stringify(followUp)}`);
  const recoveryCalls = apiCalls.slice(recoveryStartIndex);
  const followUpCalls = recoveryCalls.filter((call) => call.pathname === '/api/me/transactions');
  const authCalls = recoveryCalls.filter((call) => call.pathname === '/api/auth');
  assert(authCalls.length === 2, `expected one failed recovery auth and one later successful auth, got ${authCalls.length}`);
  assert(followUpCalls.length === 2, `expected one failed GET and one successful follow-up GET, got ${followUpCalls.length}`);
  assert(followUpCalls[1].authorization === `Bearer ${newToken}`, `follow-up GET did not use new in-memory token: ${JSON.stringify(followUpCalls)}`);
  assert(bridgeCalls.filter((method) => method === 'VKWebAppGetLaunchParams').length === 0, `post-boot auth unexpectedly refreshed launch params: ${JSON.stringify(bridgeCalls)}`);
  assert(pageErrors.length === 0, `page errors: ${pageErrors.join(' | ')}`);
  assert(consoleErrors.length === 0, `console errors: ${consoleErrors.join(' | ')}`);
  assert(failedRequests.length === 0, `failed requests: ${JSON.stringify(failedRequests)}`);

  const evidence = { ok: true, authCount, recoveryCalls, authCalls, followUpCalls, bridgeCalls, pageErrors, consoleErrors, failedRequests };
  await fs.writeFile(path.join(outDir, 'summary.json'), JSON.stringify(evidence, null, 2));
  await page.screenshot({ path: path.join(outDir, 'reauth-failure-then-success.png'), fullPage: true });
  console.log(JSON.stringify({ ok: true, authCount, newTokenUsed: true }, null, 2));
} catch (error) {
  ...
  try { await page.screenshot({ path: path.join(outDir, 'reauth-failure-then-success-failed.png'), fullPage: true }); } catch (_) {}
  throw error;
} finally {
  await context.close();
  await browser.close();
