...
if (app.includes(stateAnchor)) {
  ...
  changed = true;
} else if (!app.includes('segments: { new: 0, active: 0, at_risk: 0, sleeping: 0, no_visits: 0 }')) {
...
if (app.includes(metaAnchor)) {
  ...
  changed = true;
} else if (!app.includes("const segmentLabels = { new: 'Новые', active: 'Активные'")) {
...
if (app.includes(responseAnchor)) {
  ...
  changed = true;
} else if (!app.includes('new: Number(data.segments?.new || 0)')) {
