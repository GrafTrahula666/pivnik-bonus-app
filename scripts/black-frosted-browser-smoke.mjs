...
try {
  for (const mode of ['telegram', 'vk']) {
    ...
    for (const key of ['icon', 'bottomNav', 'vipGlass']) {
      assert(metrics[key].pointerEvents !== 'none', `${mode}: interactive ${key} is blocked by pointer-events:none`);
    }
    ...
    if (mode === 'telegram') {
      assert(/10, 12, 16/.test(metrics.icon.backgroundImage), 'telegram: icon did not resolve to shared black frosted glass');
      ...
      assert(/8, 9, 12/.test(metrics.icon.backgroundImage), 'vk: icon did not resolve to matte-black hardening layer');
      assert(!/(196, 30, 58|47, 8, 31)/.test(metrics.icon.backgroundImage), 'vk: red/burgundy tint still wins icon cascade');
    }
