...
/**
 * HTTP adapter for a future Customer 360 manual bonus endpoint.
 *
 * The adapter deliberately owns no authorization, tenant ownership or financial
 * mutation rules. The scoped authorization middleware must run first and the
 * Customer 360 runtime performs canonical customer visibility proof before
 * delegating to the shared atomic admin adjustment executor.
 */
export function createCustomerBonusAdjustmentRouteHandler({ adjust } = {}) {
  requireFunction(adjust, 'adjust');

  return async function customerBonusAdjustmentRouteHandler(req, res, next) {
    try {
...
export const customerBonusAdjustmentRouteHandlerContract = Object.freeze({
  route: '/api/spaceverse/tenants/:tenantId/locations/:locationId/customers/:id/bonus-adjustments',
