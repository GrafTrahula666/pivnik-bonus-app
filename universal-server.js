    ...
    if (await claimDataMigration(client, 'backfill-reward-grants-v2')) {
      ...
      await client.query(`
        ...
        WHERE reward_code IS NULL
          AND mode = 'adjustment'
          AND reason = 'Легендарное достижение «Пионер Пивника»'
      `);
    }
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    // VK moderation final safe 2026-08-08. General abuse protection required for public VK release.
    ...
    if (['GET', 'HEAD'].includes(req.method) && documentPlatform) {
